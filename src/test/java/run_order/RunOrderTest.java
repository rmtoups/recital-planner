...
public class RunOrderTest {
	...
	public void testFindConflicts() {
		...
		Assert.assertEquals(d2,conflicts2.get(0).getDance2());
		...
		Assert.assertEquals("d6",conflicts3.get(0).getOverlapDancers().get(0));

		// middle conflict
		Dance[] order4 = {d1,d2,d5,d3};
		RunOrder r4 = new RunOrder(order2);
		...
		Assert.assertEquals(d2,conflicts5.get(0).getDance2());
		...
		// d7,d2
		Assert.assertEquals(d1,conflicts5.get(1).getDance1());
		...
		// d3,d6
		Assert.assertEquals(d1,conflicts5.get(2).getDance1());
		Assert.assertEquals(d2,conflicts5.get(2).getDance2());
